...
class ProductsRepository implements IProductsRepository {
  ...
  public async updateQuantity(
    ...
    const storagedProducts = await this.ormRepository.findByIds(products.map(product => ({ id: product.id })));

    const updatedProducts = storagedProducts.map(product => ({
      ...product,
      quantity: product.quantity - (products.find(p => p.id === product.id)?.quantity || 0)
    }))
